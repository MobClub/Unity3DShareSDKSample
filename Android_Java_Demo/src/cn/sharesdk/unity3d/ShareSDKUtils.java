...
public class ShareSDKUtils {
	...
	public static boolean handleMessage(Message msg) {
		switch (msg.what) {
			...
			case MSG_ONEKEY_SAHRE: {
				...
				if(map.containsKey("disableSSOWhenAuthorize") && (Boolean)map.get("disableSSOWhenAuthorize")){
	...
	private static HashMap<String, Object> CSMapToJavaMap(HashMap<String, Object> content) {
		...
		try {
			boolean disableSSO = Boolean.parseBoolean(String.valueOf(content.get("disableSSOWhenAuthorize")));
			map.put("disableSSOWhenAuthorize", disableSSO);
		} catch (Exception e) {
			e.printStackTrace();
			map.put("disableSSOWhenAuthorize", false);
		}
		return map;
	}
